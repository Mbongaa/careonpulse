-- Careon Pulse — rate-limit-scope voor inloggen (handoff 13, verificatieronde).
--
-- Waarom: alle logins bereiken Supabase Auth vanaf het SERVER-IP (de login
-- loopt server-side i.v.m. de strikte CSP), dus Supabase's eigen IP-limieten
-- beschermen niet per aanvaller én een aanvaller zou juist de gedeelde limiet
-- kunnen vollopen (lock-out voor iedereen). Daarom een eigen limiet per
-- gehasht bezoekers-IP op /api/auth/login, via dezelfde atomische quota-RPC
-- als de assistent. Deze migratie voegt alleen de scope 'login' toe.
...
alter table public.careon_assistant_rate_limits
  add constraint careon_assistant_rate_limits_scope_valid
    check (scope in ('assistant', 'audit', 'login'));
...
declare
  ...
  v_retry integer := 0;
begin
  if p_scope not in ('assistant', 'audit', 'login')
