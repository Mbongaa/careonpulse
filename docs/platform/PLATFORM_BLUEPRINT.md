...
**Version:** 2.6 · **Date:** 22 August 2026 · **Status:** Approved for implementation · **Clarifies:** D14's Supabase OAuth access-token/RLS boundary and replaces raw WebView-token passing with a one-time session handoff · **Supersedes:** v2.5 (D21 calendar-write amendment) · v2.4 (D22, Entra-gated employee lifecycle and JIT membership) · v2.3 (D21, delegated Microsoft 365 data plane in YAAZ) · v2.2 (D20, hybrid authentication / Entra ID federation) · v2.1 (D19, facturatie module) · v1.0 (single-app HumHub blueprint)
...
**Phase-0 gate — status 22 Aug 2026.** The server-side OAuth protocol, ES256 validation, consent, HumHub OIDC, auto-provisioning, role claim, controlled deactivation and the public shell client/registry boundary are implemented and verified. The native shell now implements authorization-code + PKCE and secure refresh; the remaining native acceptance is a signed-device system-browser → exact app callback → registry run, followed by application-level logout/session-handoff verification. Keycloak remains the documented fallback only if that end-to-end device gate reveals a hard provider blocker; modules assume an OIDC provider, never a brand.
