...
**Version:** 2.10 · **Date:** 7 September 2026 · **Status:** Approved for implementation; **D24 (Careon Scribe) is Proposed and requires owner confirmation** · **Reconciles:** the Careon Scribe clinical live-scribe module built inside Module 1 — route `/scribe`, its own owner-bound Supabase schema, opt-in providers, and consult transcripts as a new special-category processing activity — without changing any Confirmed decision · **Supersedes:** v2.9 (D23 implementation reconciliation) · v2.8 (D23 provider and recording reconciliation) · v2.7 (D21 production reconciliation) · v2.6 (D14 OAuth access-token/RLS and one-time shell handoff clarification) · v2.5 (D21 calendar-write amendment) · v2.4 (D22, Entra-gated employee lifecycle and JIT membership) · v2.3 (D21, delegated Microsoft 365 data plane in YAAZ) · v2.2 (D20, hybrid authentication / Entra ID federation) · v2.1 (D19, facturatie module) · v1.0 (single-app HumHub blueprint)
...
**Implementation reconciliation — 7 September 2026.** D1–D23 remain accepted without redesign. This version adds **proposed decision D24** for **Careon Scribe**: live clinical conversation support built inside Module 1 at `/scribe`, with its own Supabase schema (migration `20260907120000_careon_scribe.sql`, eight owner-bound RLS tables, no Storage object) and the same four-layer shielding as D19. The module is complete in code but deliberately inert: every provider call is opt-in behind `CAREON_SCRIBE_LIVE`, the module stays off per organization until an `org_admin` enables it and authorizes clinicians, and the shell tile is withheld until the D12 phase-2 microphone permission profile exists. Consult transcripts are a **new processing activity** (special-category data, art. 9) with their own retention, processors and DPIA precondition — recorded in §18, tracked as G20 in the gap register and blocking for production activation. Nothing here changes a Confirmed decision; D24, including its D17 amendment for EU Vertex transcription, needs explicit owner confirmation. Full specification: `agent-handoff/20-clinical-scribe.md`.

**Implementation reconciliation — 5 September 2026.** D1–D23 remain accepted without redesign. [Security remediation](./SECURITY_REMEDIATION_2026-09-05.md) records account-switch isolation, a 300-second HumHub authorization lease, current-account RLS, platform-only global identity controls, atomic invoices and EPD generations. The server changes and three database migrations are deployed; shell changes are pushed with Android/iOS CI and Swift XCTest verified. [Release verification](./RELEASE_VERIFICATION_2026-09-05.md) and the [fresh browser checklist](./BROWSER_ROUTE_VERIFICATION_2026-09-05.md) record current evidence, the deployed marketplace-startup and expired-session navigation fixes, and remaining controlled acceptance. Native calling/recording/AI and other existing release gates remain unchanged.
...
**D24 was added in v2.10 and is `Proposed`, not Confirmed:** it is written in the table below so the consequences are reviewable, but no production activation, provider call or organization enablement may follow before the owner confirms it (owner confirmation required, 7 Sep 2026).
...
| D23 | Microsoft-native Teams calling in YAAZ (v2.8; implementation reconciled v2.9) | **Use Azure Communication Services Calling SDK authenticated as the employee's Teams identity** (owner-confirmed 22 Aug 2026). A TGC-owned ACS resource performs the server-side Entra→ACS token exchange. The call client receives only a short-lived ACS token in memory and never the resource connection string/key or Microsoft refresh token. The existing D21 app may request the two Microsoft-required delegated scopes `Teams.ManageCalls` and `Teams.ManageChats` through a separately gated call consent path; no application permission exists. Web delivery now provides (1) validated Outlook meeting join inside YAAZ, (2) direct and HumHub-conversation group voice/video from Messenger contacts resolved through the server-authorized Careon→Entra directory, and (3) incoming-call handling on every normal authenticated YAAZ screen while the web app is open. The later shell uses Microsoft's native iOS/Android SDKs behind Flutter platform channels. The prepared JaaS/Jitsi code and flags stay disabled as fallback until the ACS matrix is accepted. | Keeps Teams identity, policies, call history and meetings authoritative while employees remain in the Careon/YAAZ experience. Microsoft officially supports Teams-user meeting join, one-to-one/group Teams calls, incoming calls, web/iOS/Android clients and customizable UI. Consequences: ACS usage is metered; every caller needs an applicable Teams license; PSTN requires Teams Phone; web incoming calls require the YAAZ client to remain open; full Teams UI parity and force-closed native ringing are not implied. `Teams.ManageChats` is required by Microsoft's token exchange even when YAAZ messaging remains Graph/HumHub-based. Phase-3 recording/transcription is not inherited from JaaS and remains off until separately designed, consented and accepted. | Iframing the Teams client (unsupported); continuing redirect-only joins; activating the third-party JaaS provider against the owner's Microsoft-native request; exposing an ACS resource key or renewable Microsoft token to the browser |
| D24 | Careon Scribe — clinical live scribe inside Module 1 (v2.10) | **Proposed (owner confirmation required, 7 Sep 2026)** — not Confirmed; nothing is activated in production until the owner accepts this decision. **(a) Placement and access.** Careon Scribe is a route section `/scribe` with its own Supabase schema inside Module 1 (analogous to D19), not a fifth repository or OIDC client. Access is per **authorized clinician** (`careon_scribe_gemachtigden`, a D13 entitlement precursor); consult content — transcript, clinical state, report and tasks — is **owner-bound**, readable only by the clinician who conducted the consult. An `org_admin` sees session metadata without dossier reference or consult type, may delete a consult (audited, with role) and may release the **approved report only** — never the transcript — once and audited to one named colleague (offboarding). Superadmins without organization membership fall outside the module entirely; the SQL predicates carry no membership-free superadmin branch. **(b) Amendment to D17.** Transcription for the scribe may run — besides OpenAI — on Gemini through **Vertex AI in an EU region**; extraction, report generation and transcript correction stay on the existing OpenAI regime (pinned snapshot, `store:false`, strict JSON schemas, quota, content-free telemetry). **(c) EPD boundary.** The module is documentation support with a temporary working copy. The clinician transfers the approved report to the EPD themselves (copy/download, then "Overgenomen in het EPD"); Careon writes nothing into CareCheck and automates no portal form, so G06 and `docs/CARECHECK_INTEGRATION_BOUNDARY.md` are unchanged. Verbatim transcript text is a deliberate, motivated exception to that boundary's no-free-text rule, limited to the working copy and bounded by retention, owner-bound RLS and the DPIA. **(d) Browser boundary.** `Permissions-Policy: microphone=(self)` applies **only** to `/scribe*`; every entry into the module is a hard document navigation, because the policy is granted per document. **(e) DPIA.** A DPIA is a precondition for production activation. **(f) Organisation-level controls (wave 2, 7 Sep 2026).** The module switch of an organization is blocked — in the UI and in the SQL predicate `app.scribe_ingeschakeld` — until its `org_admin` has recorded the activation preconditions in the product (DPIA date and owner, confirmed processor agreement, approved consent text); external processing is a separate organisation decision on top of the platform opt-in (`transcriptieAan` for the audio route, `aiAnalyseAan` for analysis and report, both default off); the organisation reads its own scribe audit trail on `/scribe/logboek`; and clinicians can correct or retract extracted facts, which a later AI pass never undoes. **Consequences:** consult audio and transcript (art. 9 data) reach the chosen processor(s); a new processing activity enters §18; DPAs must cover the retention of API input (zero data retention / abuse-logging exclusion) per provider; everything is **opt-in** (`CAREON_SCRIBE_LIVE=1`) and off per organization until the administrator enables it and authorizes clinicians; the shell tile stays withheld until the D12 phase-2 permission profile. Full spec: `agent-handoff/20-clinical-scribe.md`; activation gates tracked as G20. | The owner's clinical live-scribe concept needs microphone capture, verbatim consult text and a clinician-approved report — none of which fits an existing module. Building it inside Module 1 keeps the CI gates on one Next.js app (D18) and leaves hosting and authentication unchanged (D15, D14), exactly as D19 did for invoicing. Gemini through Vertex AI in the EU is the only EU-region transcription option already inventoried on this platform (D6), so the amendment is named explicitly rather than letting the platform's largest personal-data stream ride silently on the assistant's existing key and DPA. | A separate repository or OIDC client (fights D18); writing the approved note into CareCheck or automating its portal forms (forbidden until a vendor contract exists); patient-identifying metadata (name, date of birth, BSN) instead of a short dossier reference; organization-wide readable consult content; a machine-written assessment/risk section or an AI diagnosis; storing audio in Supabase Storage; browser-to-provider WebSocket streaming (impossible under Vercel functions and `connect-src 'self'`); a platform-wide `microphone=(self)` header. |
...
|---|---|---|---|
| Supabase (EU) | Supabase Auth + OAuth 2.1/OIDC server; organizations, members, roles, tile entitlements; Pulse dashboard schema (RLS) incl. facturatie (D19) and the Careon Scribe schema (D24 proposed — eight owner-bound RLS tables: sessions, segments, clinical state, notes, tasks, settings, authorizations, releases; **no Storage object**, consult audio is never stored); audit + rate-limit tables; Storage bucket `facturen` (private — final invoice PDFs + org logo, own backup regime) | SQL migrations in `careonpulse/supabase` | live |
...
**Microsoft Teams call plane (Confirmed, D23 — v2.8; implementation reconciled v2.9).** Calling is not implemented with Microsoft Graph and never uses the D20 Supabase login token. YAAZ obtains an Entra access token for the dedicated ACS resource scopes, proves tenant/user/client continuity against the existing D21 connection, and exchanges it server-side through the TGC-owned ACS resource. Only the returned short-lived Teams-user ACS token crosses to the active web client, under `no-store`; it is held in JavaScript memory and never written to MariaDB, browser storage, a URL or logs. The ACS resource credential stays in the deployment secret store. A read-only Careon bridge exposes only eligible Careon subject→Entra object-ID pairs so YAAZ can authorize direct/group targets without leaking names, e-mail, roles or tokens. The web client uses `createTeamsCallAgent()` for exact validated meetings and server-authorized employee/group targets. A listener outside the PJAX content remains initialized across normal authenticated YAAZ screens, while camera/microphone permission is requested only after start or accept. Browser-closed and force-closed background ringing remain native shell increments.
...
**Client presentation update — 10 September 2026.** The employee launcher shows **Careon Dashboard**, **YAAZ**,
**Careon Academie/Academy**, **Careon AI**, and **Careon Kwaliteitshandboek**. The last three are non-launchable
"Binnenkort beschikbaar" previews. Administrators additionally retain **Facturatie** under D19. Careon AI is
the product name for the existing Careon Scribe implementation; `careon-scribe`, `/scribe`, its API/schema
identifiers and all D24 activation and authorization gates remain unchanged. Displaying a preview does not
grant entitlement to use a module. Historical references to Careon Scribe below name that same implementation.
...
| Clinical live scribe (consult transcription, structured state, approved note) | Built inside Module 1 (D24 proposed, opt-in and off per organization) | Built per `agent-handoff/20-clinical-scribe.md`; awaiting D24 confirmation, DPA incl. API-input retention/ZDR per provider, and a DPIA before activation | 1 |
...
The backend exchanges a call-scoped Entra access token through ACS using the employee's verified Entra object ID and the configured client ID. The browser receives `{token, expiresOn, mode}` plus only the exact revalidated meeting or direct/group Entra targets after server authorization. Direct/group resolution starts with local HumHub conversation membership and an exact Careon OIDC subject, then calls the fixed read-only Careon directory endpoint; client-supplied Entra IDs are never accepted. Responses are `no-store`; secrets and tokens are excluded from logs and browser storage. The resource connection string/key never crosses the server boundary.
...
YAAZ uses the ACS Calling JavaScript SDK in both a dedicated call surface and a hidden app-wide listener outside HumHub's PJAX content. The listener creates a `TeamsCallAgent` from a short-lived token on every normal authenticated YAAZ page and remains registered while employees move between modules; no microphone/camera prompt occurs until start or accept. The visible call UI renders pre-call, ringing, connected, reconnecting and ended states. A scheduled meeting uses only the server-revalidated exact Teams join URL. Direct/group calls use only server-authorized `microsoftTeamsUserId` targets; the browser cannot supply an arbitrary Entra ID.

### 10.2 Messenger and contacts

The **Open chats / Contacten** switch exposes all currently messageable YAAZ Space coworkers. Selecting a contact opens its existing exact two-person conversation or an empty WhatsApp-style draft with no subject; the HumHub conversation is created only when the first message is sent. Group creation selects 2–50 contacts and likewise opens empty. Voice/video actions appear in contacts, member cards/profiles and direct/group conversation headers when every call target resolves to an enabled, licensed Entra member. Group calls reuse the exact authorized HumHub participant set. HumHub chat remains HumHub and Teams chat remains Graph; the Microsoft-required `Teams.ManageChats` consent is not used to silently migrate or duplicate conversations.
...
While any normal authenticated YAAZ web screen remains open, the persistent call client exposes `incomingCall` through an app-wide ringing panel with accept/reject and the Microsoft-supplied caller display name where available. Leaving the dedicated call screen forces a clean navigation so the global listener is restored rather than lost to PJAX shell reuse. Reliable browser-closed/background/force-closed ringing belongs to the native increment: Swift and Kotlin ACS SDK adapters, CallKit/PushKit on iOS, and the reviewed Android foreground/full-screen notification contract. Flutter invokes those adapters through narrow platform channels; no resource credential or renewable Microsoft token enters Dart.

### 10.5 Supported boundary

Phase 2 includes Teams-user VoIP direct/group voice/video, scheduled/channel meeting join, incoming calls while YAAZ is open, mute/unmute, device selection, video, screen sharing where the chosen SDK/platform supports it, hang-up and reconnection. It does not promise the full Teams UI, live events, joining an already-running unscheduled Teams 1:1/group call, ringing after the web app is closed, background-native ringing in the first web release, PSTN/Teams Phone, call queues, recording, transcription or AI. Microsoft Teams policies and licensing remain authoritative.
...
**Careon Scribe — new processing activity (D24 proposed, 7 September 2026).** The clinical live scribe introduces the platform's most sensitive processing and is therefore listed separately in this inventory. *What is processed:* verbatim consult transcript segments and the derived clinical state, report versions and follow-up tasks. Transcript text is **special-category data (art. 9 GDPR)** and can be directly identifying because names, dates of birth or addresses may be spoken aloud — a deliberate, documented exception to the dashboard's pseudonymization line and to the no-free-text rule of `docs/CARECHECK_INTEGRATION_BOUNDARY.md`, limited to the temporary working copy. Session metadata carries only a short **dossier reference** (validated: no BSN, no date of birth, no name) plus the frozen consent text, its settings revision and the confirmation timestamp. *Audio:* never stored by Careon — self-contained fragments are relayed to the transcription provider and discarded; there is no Storage bucket, no temporary file and no audio or transcript text in any log. *Processors:* OpenAI for extraction, report generation and (optionally) transcription, and — only when the `gemini` provider is selected — Google Cloud (Vertex AI, EU region) as a **new processor** with its own DPA. Both are inert until `CAREON_SCRIBE_LIVE=1` **and** the organization's own external-processing switches (`transcriptieAan`, `aiAnalyseAan`, default off) are on; consult audio and text are new material relative to the existing assistant DPA line, so per-provider agreements must cover the retention of API input (zero data retention and exclusion from abuse logging). *Retention:* transcript and clinical state follow the shortest applicable term — erased synchronously on transfer to the EPD and on cancellation, otherwise after the organization's transcript retention (default 30 days); the report and session metadata are removed with the daily prune at most 24 hours after their term (default 30 days after transfer, 0 = immediately). Audit events keep 365 days, content-free provider telemetry 90 days. *Access:* owner-bound RLS — only the treating clinician reads consult content; an `org_admin` sees metadata without dossier reference or consult type, may delete, and may release the approved report (never the transcript) once and audited to one named colleague; superadmins without organization membership are excluded. *Lawful basis and safeguards:* the consult cannot start without a confirmed consent statement whose literal text and revision are frozen per consult (art. 7(1) demonstrability); a **DPIA must be established before production activation**; the module stays off per organization until an administrator enables it and authorizes clinicians. Open owner questions are in §25; activation gates are tracked as G20.
...
**Environment variables — dashboard plane (Careon Scribe, D24 proposed).** All values are read **server-side only** and never carry a `NEXT_PUBLIC_` prefix; they live in the Vercel secret store. While `CAREON_SCRIBE_LIVE` is unset the module makes no provider call at all: transcription answers 503 and analysis/report generation run deterministically. Go-live checklist: `PRODUCTION_MODE.md`; full contract: `.env.example`.

| Variable | Purpose |
|---|---|
| CAREON_SCRIBE_LIVE | Opt-in kill switch for **every** provider call (`1` enables). Absent/`0` keeps the module fully deterministic; it does not by itself enable the module for an organization |
| CAREON_SCRIBE_TRANSCRIPTION_PROVIDER | `openai` (within D17) or `gemini` (Vertex AI EU, D24b). Empty or incompletely configured fails closed with 503 and manual text entry |
| CAREON_SCRIBE_TRANSCRIPTION_MODEL | Transcription model for the OpenAI adapter (default `gpt-4o-mini-transcribe`) |
| CAREON_SCRIBE_VERTEX_PROJECT_ID / CAREON_SCRIBE_VERTEX_LOCATION | Vertex AI project and EU region (`europe-west4`) for the `gemini` adapter |
| CAREON_SCRIBE_GEMINI_MODEL | Mandatory, never hard-coded in code (D6) |
| CAREON_SCRIBE_VERTEX_SERVICE_ACCOUNT_JSON | Base64 service-account credential for the Vertex token exchange; a secret of the same order as the service-role key |
| CAREON_SCRIBE_RATE_LIMIT_PER_MINUTE / _PER_DAY | Per-user quota on the `scribe` scope (40 / 6,000), fail-closed on an RPC outage |
| CAREON_SCRIBE_RATE_LIMIT_ORG_PER_DAY | Per-organization daily ceiling on the `scribe` scope (20,000) |
...
21. *(Careon Scribe, D24 — added 7 September 2026)* May consult audio and verbatim consult text be sent to OpenAI, **and is zero data retention / exclusion from abuse logging contractually recorded**? — *Default until answered: provider and AI stay off (`CAREON_SCRIBE_LIVE` empty).*
22. *(Careon Scribe, D24 — added 7 September 2026)* Gemini through Vertex AI (EU) as transcription provider (D24b), with the same zero-data-retention question, or OpenAI only? — *Default until answered: off.*
23. *(Careon Scribe, D24 — added 7 September 2026)* Consent wording, to be reviewed with the client's counsel: does spoken consent plus a confirmed checkbox suffice for the healthcare context? — *Default: the standard text in `agent-handoff/20-clinical-scribe.md` §4.6, frozen per consult with its settings revision.*
24. *(Careon Scribe, D24 — added 7 September 2026)* Who establishes the DPIA for the consult-transcript processing, and when? — *Blocking for production activation.*
