...
Shell app configuration (Supabase URL, OIDC client id, tile-registry endpoint) is build-time config in `careonpulse-shell`; dashboard configuration stays defined by the repo's own `.env.example`.
...
9. Is SSO federation to an external IdP (Azure AD, etc.) needed later for any organization?
