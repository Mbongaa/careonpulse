...
|---|---|---|---|---|---|---|
| G01 | Employee onboarding | Production Directie reconciles all 66 Entra identities and separately marks the 3 users assigned to `Careon.User`. Microsoft is the prominent default login for the whole Careon dashboard; existing-account linking and a disposable new employee's automatic Careon identity + default `member` creation are production-accepted. YAAZ first-launch creation and the remaining live denial matrix are still outstanding. | The Careon dashboard path is scalable for assigned employees, but full cross-module onboarding acceptance and broader eligibility governance are not finished. | Entra-gated, audited employee provisioning with safe default access, clear role ownership and reliable offboarding. | P0 | In progress |
...
- **21 Aug 2026 — new-employee Careon JIT and G15 offboarding production-accepted:** tenant-wide admin consent is granted for only the login app's delegated `email` and `User.Read` scopes. A disposable active tenant member was directly assigned `Careon.User`; its first Microsoft login created exactly one Supabase Azure identity, one TGC membership with role `member`, and one `org.user.jit_provision` audit event, then landed on `/modules`. `/api/auth/session` proved no superadmin, financial or invoicing access, and an attempted `/dashboard/beheer` visit redirected to the ordinary dashboard. The acceptance run exposed and fixed a current-PostgREST compatibility defect: migration `20260821155300_current_postgrest_jwt_role_guard` now reads the signed role through `auth.jwt()` while preserving service-role-only grants; the same guard protects both lifecycle RPCs. The first complete snapshot observed 67 identities with 4 eligible, 3 managed ordinary members, 1 exempt organization administrator, no actions and no errors. Disabling the disposable employee then produced exactly one successful `account_disabled` offboarding action; the existing Careon session immediately returned `authed:false`, the Auth identity was banned, the lifecycle/audit rows finalized without error and Microsoft rejected sign-in as locked. No real employee was offboarded. The disposable Entra user was deleted after acceptance (recoverable for 30 days), its temporary Careon identity/membership was removed, and the final clean snapshot returned 66 identities, 3 eligible assignments, 3 active Careon matches, 0 pending first logins, 7 guests and 7 disabled accounts. G15 is closed; G01 remains open only for YAAZ first-launch provisioning plus the remaining live denial cases.
