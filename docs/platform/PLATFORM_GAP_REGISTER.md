...
| G16 | Reproducible source and deployment | The reviewed D20–D22 implementation is committed and pushed. The current production security release is Careon `a47399e` and platform `6c2c813`; production module trees equal their tracked sources. Three unrelated owner edits remain deliberately unstaged in the Careon worktree. | The former overwrite/reconstruction risk is closed; future production changes still require the same commit-first discipline. | Commit/review all intended changes, remove drift, pin versions/config and prove a clean-source deployment. | P0 | Done |
