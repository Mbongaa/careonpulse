...
|---|---|---|---|---|---|---|
| G01 | Employee onboarding | Directie can now reconcile all 66 Entra identities and separately marks the 3 users assigned to `Careon.User`; assignment-gated JIT provisioning and the production connectors are configured, but a real first-login acceptance is still outstanding. | The scalable identity path exists, but the broader workforce is not yet eligible or accepted for rollout. | Entra-gated, audited employee provisioning with safe default access, clear role ownership and reliable offboarding. | P0 | In progress |
...
| G15 | Offboarding/session revocation | The fail-closed hourly reconciler, Careon ban boundary and YAAZ account/Graph disconnect endpoint are built; YAAZ 0.4.0 is live and the Careon activation is awaiting its final deployment and a controlled real-user acceptance. | Until live acceptance is complete, the cross-plane offboarding SLA is implemented but not operationally proven. | Automated eligibility reconciliation, immediate Careon block/session revocation, Graph disconnect and evidenced SLA. | P0 | In progress |
