...
| G16 | Reproducible source and deployment | D20/D21 live work includes modified/untracked state across repositories; Git deploy may overwrite it and origin/main is not a complete production source. | Production cannot be reconstructed or safely evolved from source control. | Commit/review all intended changes, remove drift, pin versions/config and prove a clean-source deployment. | P0 | Open |
