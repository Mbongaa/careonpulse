...
function ToolFallbackTrigger({
  ...
  const label = isCancelled ? "Tool geannuleerd" : "Tool gebruikt";

  return (
    <CollapsibleTrigger
      data-slot="tool-fallback-trigger"
      className={cn("group/trigger flex w-full items-center gap-2 px-4 text-sm transition-colors", className)}
      {...props}
    >
...
function ToolFallbackResult({ result, className, ...props }: React.ComponentProps<"div"> & { result?: unknown }) {
  ...
  return (
    <div data-slot="tool-fallback-result" className={cn("border-t border-dashed px-4 pt-2", className)} {...props}>
      <p className="font-semibold">Resultaat:</p>
...
function ToolFallbackError({
  ...
  const headerText = isCancelled ? "Reden van annulering:" : "Fout:";
