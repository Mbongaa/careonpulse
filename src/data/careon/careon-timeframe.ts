...
export type CareonTimeframe = "12m" | "6m" | "3m" | "1m";
...
/** Laatste N punten van een chronologisch gesorteerde maandreeks. */
export function sliceTimeframe<T>(series: readonly T[], timeframe: CareonTimeframe): T[] {
  return series.slice(-CAREON_TIMEFRAME_MONTHS[timeframe]);
}
...
export function timeframeKeys(sortedKeys: readonly string[], timeframe: CareonTimeframe): Set<string> {
  return new Set(sortedKeys.slice(-CAREON_TIMEFRAME_MONTHS[timeframe]));
}
